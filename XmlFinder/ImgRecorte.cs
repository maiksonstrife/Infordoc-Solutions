...
namespace XmlFinder
{
    class ImgRecorte
    {
        ...
        public static void  PDFVertical (Bitmap originalImage, string pdffile, string in_folder, int partes)
        {
            ...
            //configurando loop
            int r = originalImage.Width / partes; //decidindo posicao X, exemplo: imagem 800x800, 800/4 = 200(posicao inicial)
            int i = 0;
            //loop
            for (i = 0; i < partes - 1; i++) {  //1 - se começa em 200 termina em 400, 2- se comeca em 400 termina em 600, 3 - se começa em 600 termina em 800

                //recortando bmp  metade 2
                rect = new Rectangle(r, 0, originalImage.Width / 2, originalImage.Height);
                ...
                saidaFilename = in_folder + @"\" + Path.GetFileNameWithoutExtension(pdffile).ToString() + "_2.pdf";
        ...
        public static void PDFVertical4doc(Bitmap originalImage, string pdffile, string in_folder)
        {
            string saidaFilename;
            PdfDocument pdf = PdfReader.Open(pdffile, PdfDocumentOpenMode.Import);
            ...
            out_pdf.Info.Creator = "Infordoc";
            saidaFilename = in_folder + @"\" + Path.GetFileNameWithoutExtension(pdffile).ToString() + "_1.pdf";
            imgrecorte.Add_new_page(firstHalf, out_pdf); //adiciona pagina 0 (a unica)
            out_pdf.Save(saidaFilename);
            out_pdf.Close();
            ...
            out_pdf2.Info.Creator = "Infordoc";
            saidaFilename = in_folder + @"\" + Path.GetFileNameWithoutExtension(pdffile).ToString() + "_2.pdf";
            imgrecorte.Add_new_page(secondHalf, out_pdf2); //adiciona pagina 0 (a unica)
            out_pdf2.Save(saidaFilename);
            out_pdf2.Close();
        }
